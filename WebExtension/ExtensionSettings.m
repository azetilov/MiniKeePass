...
@implementation ExtensionSettings
...
static ExtensionSettings *sharedInstance;

+ (void)initialize {
    static BOOL initialized = NO;
    if (!initialized) {
        initialized = YES;
        sharedInstance = [[ExtensionSettings alloc] init];
    }
}

+ (ExtensionSettings *)sharedInstance {
...
- (id)init {
    self = [super init];
    if (self) {
        userDefaults = [[NSUserDefaults alloc] initWithSuiteName: APP_GROUP_ID];
